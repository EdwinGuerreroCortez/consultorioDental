...
import {
  ...
  Alert, Box, Divider, Chip, Paper, Container, IconButton, Dialog, DialogActions, DialogContent, DialogTitle, TextField, Button,InputAdornment 
...
const ProfileCard = styled(motion(Card))(({ theme }) => ({
  maxWidth: 850,
  margin: '4rem auto',
  borderRadius: '20px',
  padding: '2.5rem',
...
const InfoItem = styled(motion(Paper))(({ theme }) => ({
  display: 'flex',
  alignItems: 'center',
...
const handleSubmit = async () => {
      ...
      <ProfileCard initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <Grid container spacing={2}>
          {[
            ...
            {
              icon: <CalendarToday sx={{ color: "#56CCF2" }} />, label: "Fecha de Nacimiento",
  ...
  <DialogContent>
    ...
    <Box sx={{ mt: 2 }}>
      <Typography variant="body2" color={passwordStrength.score < 2 ? "error" : passwordStrength.score < 4 ? "warning" : "success"} sx={{ fontWeight: "bold" }}>
  ...
  <DialogActions>
    ...
    <Button onClick={handleSubmit} color="primary" variant="contained">Actualizar</Button>
  </DialogActions>
</Dialog>
